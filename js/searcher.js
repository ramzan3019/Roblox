...
const userInput = document.getElementById("RE-UserInput");
const searchButton = document.getElementById("RE-SearchButton");
const warningContainer = document.getElementById("RE-PSA-Container");
const warningText = document.getElementById("RE-PSA-Warning");
...
searchButton.addEventListener("click", async () => {
    await chrome.storage.local.get(null, async (RE) => {
        if (!RE.EULA) return showEULA(RE);
        disableInptBtn(true);
...
function showEULA(RE) {
    fetch(chrome.runtime.getURL("resources/html/eula.html")).then(result => result.text()).then(html => {
        ...
        const agreeCheckbox = document.getElementById("RE-AGMT-Agree");
        const countdown = document.getElementById("RE-AGMT-Countdown");
...
async function findPlayer() {
    ...
    const searchButtonAvatar = document.getElementById("RE-SearchButtonAvatar");
