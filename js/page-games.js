...
(async () => {
    ...
    if (winLocHash[1] && winLocHash[1].startsWith("re_join")) {   // check direct join
    ...
    function createLayout() {
        ...
        function appendLayout() {
            fetch(chrome.runtime.getURL("resources/html/layout.html")).then(result => result.text()).then(RElayout => {
                const layout = document.createElement("div");
                const serverListOptions = document.getElementsByClassName("server-list-options")[0];
                serverListOptions.append(layout);
                layout.outerHTML = RElayout;
                PROGRESS_BAR = document.getElementById("RE-ProgressBar");
                
                chrome.runtime.sendMessage({ msg: "RE_Run_searcher.js" });
            });
        }
    }
})();